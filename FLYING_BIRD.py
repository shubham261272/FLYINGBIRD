...
# background image
BG_IMG = pygame.image.load('bgshubham.jpg')

# Setting the display
DIS = pygame.display.set_mode((500, 750))

# x-axis and y-axis movement of bird
BIRD_IMAGE = pygame.image.load('bird.gif')
BRD_x = 50
BRD_y = 300
BRD_y_CH = 0

def BIRD_MOVEMENT(x, y):
    DIS.blit(BIRD_IMAGE, (x, y))

# RACT-SHAPE
RACT_SHAPE = 40
RACT_HEIGHT = random.randint(150,450)
RACT_X_CHANGE = -0.5
RACT_x = 500
RACT_COLOR = (7,9,40)

def DISPLAY_RACT(height):
    pygame.draw.rect(DIS, RACT_COLOR, (RACT_x, 0,RACT_SHAPE, height))
    bottom_obstacle_height = 700 - height - 150
    pygame.draw.rect(DIS,RACT_COLOR, (RACT_x,height+150,RACT_SHAPE,bottom_obstacle_height))
...
while running:
    ...
    # Moving the obstacle
    RACT_x += RACT_X_CHANGE

    # COLLISION
    collision = collision_detection(RACT_x,RACT_HEIGHT, BRD_y, RACT_HEIGHT + 150)
    ...
    if RACT_x <= -10:
        RACT_x = 500
        RACT_HEIGHT= random.randint(200, 400)
        score += 1
    # displaying the obstacle
    DISPLAY_RACT(RACT_HEIGHT)

    # displaying the bird
    BIRD_MOVEMENT(BRD_x, BRD_y)
